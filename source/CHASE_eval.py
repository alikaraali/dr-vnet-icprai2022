...
testing_images_loc = '../Chase/test/image/'
testing_label_loc = '../Chase/test/label/'
...
for i in test_files:
    ...
    label = cv2.imread(testing_label_loc + "Image_" + i.split('_')[1].split(".")[0] + "_1stHO.png", cv2.IMREAD_GRAYSCALE)
...
for train_no in range(1, 6):
    ...
    for y in y_pred:
        ...
        cv2.imwrite('../results/test/chase/{:d}/{:d}.png'.format(train_no, i), tempx.astype(np.uint8))
        np.save('../results/test/chase/{:d}/{:d}.npy'.format(train_no, i), y)
